    ...
    for (y = 0; y < YRES / CELL; y++) //update velocity and pressure
    {
        for (x = 0; x < XRES / CELL; x++)
        {
            ...
            if (i > 2 && i < XRES / CELL - 3 && j > 2 && j < YRES / CELL - 3)
            {
                odh = dh;
                dh *= 1.0f - AIR_VADV;
                dh += AIR_VADV * (1.0f - tx) * (1.0f - ty) * ((bmap_blockairh[j - 1][i - 1] & 0x8) ? odh : hv[j - 1][i - 1]);
                dh += AIR_VADV * tx * (1.0f - ty) * ((bmap_blockairh[j - 1][i + 1] & 0x8) ? odh : hv[j - 1][i + 1]);
                dh += AIR_VADV * (1.0f - tx) * ty * ((bmap_blockairh[j + 1][i - 1] & 0x8) ? odh : hv[j + 1][i - 1]);
...
void Air::update_air(void)
{
    ...
    if (airMode != 4) { //airMode 4 is no air/pressure update
        for (i = 0; i < YRES / CELL; i++) //reduces pressure/velocity on the edges every frame
        {
            ...
            pv[i][1] = pv[i][1] * 0.8f;
            pv[i][2] = pv[i][2] * 0.8f;
            pv[i][XRES / CELL - 3] = pv[i][XRES / CELL - 3] * 0.8f;
        ...
        for (i = 0; i < XRES / CELL; i++) //reduces pressure/velocity on the edges every frame
        {
            ...
            pv[1][i] = pv[1][i] * 0.8f;
            pv[2][i] = pv[2][i] * 0.8f;
            pv[YRES / CELL - 3][i] = pv[YRES / CELL - 3][i] * 0.8f;
        ...
        for (y = 1; y < YRES / CELL - 1; y++) //pressure adjustments from velocity
            for (x = 1; x < XRES / CELL - 1; x++)
            {
                if (bmap_blockair[y][x]) continue;
                dp = 0.0f;
        ...
        for (y = 1; y < YRES / CELL - 1; y++) //velocity adjustments from pressure
            for (x = 1; x < XRES / CELL - 1; x++)
            {
                if (bmap_blockair[y][x]) continue;
                dx = dy = 0.0f;
                dx += (bmap_blockair[y][x - 1] || bmap_blockair[y][x + 1]) ? 0 : (pv[y][x - 1]) - (pv[y][x + 1]);
                dy += (bmap_blockair[y - 1][x] || bmap_blockair[y + 1][x]) ? 0 : (pv[y - 1][x]) - (pv[y + 1][x]);
                vx[y][x] *= AIR_VLOSS;
                vy[y][x] *= AIR_VLOSS;
        ...
        for (y = 0; y < YRES / CELL; y++) //update velocity and pressure
            for (x = 0; x < XRES / CELL; x++)
            {
                ...
                dv = std::sqrtf(dx * dx + dy * dy);
                tx = x + 0.5f - dx * advDistanceMult;
                ty = y + 0.5f - dy * advDistanceMult;
                if ((dv > 1.0f) && (tx >= 2 && tx <= XRES / CELL - 2 && ty >= 2 && ty <= YRES / CELL - 2))
                {
                    ...
                    stepX = -dx / dv;
                    stepY = -dy / dv;
                    stepLimit = advDistanceMult * dv;
                    //Setting to the center of the block...
                    tx = x + 0.5f;
                    ty = y + 0.5f;
                    for (step = 0; step < stepLimit; ++step)
                    {
                        tx += stepX;
                        ty += stepY;
                        if (!(tx >= 2 && tx < XRES / CELL - 2 && ty >= 2 && ty < YRES / CELL - 2)
                            || bmap_blockair[(int)std::floorf(ty)][(int)std::floorf(tx)])
                        {
                            tx -= stepX;
                            ty -= stepY;
                            break;
                ...
                tx -= i;
                ty -= j;
                if (!bmap_blockair[y][x] && i >= 3 && i <= XRES / CELL - 3 &&
                    j >= 3 && j <= YRES / CELL - 3)
                {
                    dx *= 1.0f - AIR_VADV;
                    dy *= 1.0f - AIR_VADV;
#define lerp(a,b,t) (a + (b - a) * t)
#define lerpAves(a,b,c,t) lerp(0.5f*(a+b),0.5f*(b+c),t)
#define bavx(yk,xk,yd,xd) (bmap_blockair[yk][xk] ? vx[yd][xd] : vx[yk][xk])
#define bavy(yk,xk,yd,xd) (bmap_blockair[yk][xk] ? vy[yd][xd] : vy[yk][xk])
                    dx += AIR_VADV * lerpAves(
                        lerpAves(bavx(j - 1, i - 1, j, i), bavx(j - 1, i, j, i), bavx(j - 1, i + 1, j, i), tx),
                        lerpAves(bavx(j, i - 1, j, i), bavx(j, i, j, i), bavx(j, i + 1, j, i), tx),
                        lerpAves(bavx(j + 1, i - 1, j, i), bavx(j + 1, i, j, i), bavx(j + 1, i + 1, j, i), tx),
                        ty);
                    dy += AIR_VADV * lerpAves(
                        lerpAves(bavy(j - 1, i - 1, j, i), bavy(j - 1, i, j, i), bavy(j - 1, i + 1, j, i), tx),
                        lerpAves(bavy(j, i - 1, j, i), bavy(j, i, j, i), bavy(j, i + 1, j, i), tx),
                        lerpAves(bavy(j + 1, i - 1, j, i), bavy(j + 1, i, j, i), bavy(j + 1, i + 1, j, i), tx),
                        ty);
                    /*dx += AIR_VADV * (
                        0.5f * (1 - ty) * (0.5f * (1 - tx) * bavx(j - 1, i - 1, j, i) + 0.5f * bavx(j - 1, i, j, i) + 0.5f * tx * bavx(j - 1, i + 1, j, i))
                        + 0.5f * (0.5f * (1 - tx) * bavx(j, i - 1, j, i) + 0.5f * bavx(j, i, j, i) + 0.5f * tx * bavx(j, i + 1, j, i))
                        + 0.5f * ty * (0.5f * (1 - tx) * bavx(j + 1, i - 1, j, i) + 0.5f * bavx(j + 1, i, j, i) + 0.5f * tx * bavx(j + 1, i + 1, j, i)));
                    dy += AIR_VADV * (
                        0.5f * (1 - ty) * (0.5f * (1 - tx) * bavy(j - 1, i - 1, j, i) + 0.5f * bavy(j - 1, i, j, i) + 0.5f * tx * bavy(j - 1, i + 1, j, i))
                        + 0.5f * (0.5f * (1 - tx) * bavy(j, i - 1, j, i) + 0.5f * bavy(j, i, j, i) + 0.5f * tx * bavy(j, i + 1, j, i))
                        + 0.5f * ty * (0.5f * (1 - tx) * bavy(j + 1, i - 1, j, i) + 0.5f * bavy(j + 1, i, j, i) + 0.5f * tx * bavy(j + 1, i + 1, j, i)));*/
#undef lerp(a,b,t)
#undef lerpAves(a,b,c,t)
#undef bavx(xk,yk,xd,yd)
#undef bavy(xk,yk,xd,yd)
                }
